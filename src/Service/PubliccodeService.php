...
/**
 *  This class handles the publiccode file.
 *
...
class PubliccodeService
{
    ...
    public function handleMaintenaceObjects(ObjectEntity $component, array $itemArray, string $valueName): ObjectEntity
    {
        ...
        if ($maintenanceSchema instanceof Entity === false) {
            return $component;
        }
    ...
    public function handleContractors(Source $source, ObjectEntity $component, array $publiccode): ObjectEntity
    {
        ...
        foreach ($publiccode['maintenance']['contractors'] as $contractor) {
            ...
            if (key_exists('name', $contractor) === true && key_exists('until', $contractor) === true) {
                ...
                if ($organizationSchema instanceof Entity === false && $contractorSchema instanceof Entity === false) {
                    return $component;
                }
     ...
     * @return ObjectEntity The updated component object.
     */
    public function handleLegal(Source $source, ObjectEntity $component, array $publiccodeArray): ObjectEntity
    {
        $organizationSchema = $this->resourceService->getSchema($this->configuration['organizationSchema'], 'open-catalogi/open-catalogi-bundle');
    ...
    public function handlePubliccodeSubObjects(array $publiccodeArray, Source $source, ObjectEntity $component, array $publiccode): ObjectEntity
    {
        ...
        if (key_exists('legal', $publiccodeArray) === true) {
            $component = $this->handleLegal($source, $component, $publiccodeArray);
        }//end if
        ...
        if (key_exists('applicationSuite', $publiccodeArray) === true
            ...
            if ($applicationSchema instanceof Entity === false) {
                return $component;
            }
     ...
     */
    public function getError(Source $source, array $publiccodeArray, int $errorCode, string $type, string $endpoint): ?string
    {
     ...
     * @return string|null With type raw the logo from the publiccode file if valid, if not null is returned. With type url and relative the download_url from the reponse of the call.
     */
    public function getResponse(Response $response, Source $source, string $logoUrl, string $type): ?string
    {
    ...
    public function getLogoFileContent(array $publiccodeArray, Source $source, string $endpoint, string $type, ?string $logoUrl=null, ?array $queryParam=[]): ?string
    {
        ...
        if (isset($response) === false) {
            return $this->getError($source, $publiccodeArray, $errorCode, $type, $endpoint);
        }//end if

        if (isset($response) === true) {
            return $this->getResponse($response, $source, $logoUrl, $type);
        }//end if
    ...
    public function handleLogoFromGithub(array $publiccodeArray, Source $source, array $parsedLogo, string $repositoryName): ?string
    {
        ...
        if (str_contains($explodedPath, '/blob/') === true) {
            ...
            for ($i = 0; $i < count($explodedPath); $i++) {
                // If i is <= 2 then nothing is done.
                // The /blob/{branch} will not be added to the path.
                if ($i <= 2) {
                    continue;
                }

                // If i is not <= 2 then the $explodedPath item is set to the $path.
                $path .= '/'.$explodedPath[$i];
            }
        }//end if
    ...
    public function handleLogoFromGitlab(array $componentArray, Source $source, array $parsedLogo, string $repositoryName, string $repositoryId): ?string
    {
        ...
        if (str_contains($parsedLogo['path'], '/uploads/') === false
            ...
            $branch = 'main';
            for ($i = 0; $i < count($explodedPath); $i++) {
                // If i is <= 3 then nothing is done.
                // The /-/blob/{branch} will not be added to the path.
                if ($i <= 2) {
                    continue;
                }
     ...
     */
    public function switchLogo(array $componentArray, Source $source, string $repositoryId, string $repositoryName): ?string
    {
    ...
    public function handlePubliccodeFile(array $publiccodeArray, Source $source, ObjectEntity $repository, array $data, ?array $repositoryArray=[]): ?ObjectEntity
    {
        ...
        if ($publiccodeMapping instanceof Mapping === false || $componentSchema instanceof Entity === false) {
            return $repository;
        }
        ...
        if ($this->syncService->doesShaMatch($componentSync, $sha) === true) {
            ...
            $this->pluginLogger->info('The sha is the same as the sha from the component sync. The given sha (publiccode url from the github api)  is: '.$sha);
